...
O termo AIOps (*Artificial Intelligence for IT Operations*) foi cunhado pelo Gartner em 2016 e consolidado como categoria de mercado em seu *Market Guide for AIOps Platforms* [Prasad e Rich, 2018]. O conceito descreve a aplicação de técnicas de inteligência artificial e aprendizado de máquina para automatizar e aprimorar processos de operações de TI, incluindo correlação de eventos, detecção de anomalias e determinação de causalidade [Notaro et al., 2021].

Zhang et al. (2025) ampliam essa definição ao contextualizar o AIOps no cenário atual de sistemas distribuídos em larga escala:

> *"AIOps refers to the use of machine learning (ML) and deep learning (DL) techniques to process vast amounts of data generated by operational tools and infrastructure, enabling the real-time detection, diagnosis, and resolution of system issues."* [Zhang et al., 2025, p. 2]

A motivação central para o AIOps reside na incapacidade humana de processar, em tempo real, o volume massivo de dados de telemetria gerado por sistemas modernos. Wang et al. (2025) descrevem esse cenário ao afirmar que o volume de dados de telemetria — métricas, logs e rastros distribuídos — gerado por arquiteturas de microsserviços representa uma carga cognitiva substancial para operadores humanos, que frequentemente precisam correlacionar fontes de dados díspares, identificar causas raiz e implementar ações de remediação sob condições de alta pressão temporal.

---

## 2.2.2 Taxonomia de tarefas em AIOps

A literatura organiza as tarefas de AIOps em três estágios sequenciais e interdependentes, conforme sistematizado por Zhang et al. (2025) e ilustrado na Figura 2.X:

```
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│ Failure          │     │ Root Cause       │     │ Assisted         │
│ Perception       │────►│ Analysis         │────►│ Remediation      │
│                  │     │                  │     │                  │
│ Detecção de      │     │ Diagnóstico e    │     │ Remediação       │
│ anomalias        │     │ identificação    │     │ assistida ou     │
│                  │     │ de causas raiz   │     │ automatizada     │
└──────────────────┘     └──────────────────┘     └──────────────────┘
```

*Figura 2.X: Taxonomia de tarefas em AIOps — adaptado de Zhang et al. (2025).*

**Failure Perception** (Percepção de Falhas) é o estágio inicial, responsável por detectar se anomalias ocorreram no sistema. Abrange subtarefas como prevenção de falhas, predição de falhas e detecção de anomalias — sendo esta última a mais estudada na literatura, tanto antes quanto após o advento dos LLMs [Zhang et al., 2025]. Neste estágio, o Prometheus desempenha papel central no presente trabalho, monitorando continuamente as métricas das instâncias EC2 e disparando alertas quando limiares predefinidos são excedidos.

**Root Cause Analysis** (Análise de Causa Raiz — RCA) é o estágio intermediário e foco do presente trabalho. Uma vez detectada uma anomalia, este estágio identifica a localização e a natureza do problema por meio de análise automatizada [Zhang et al., 2025]. O estágio subdivide-se em três subtarefas: localização da falha (*Failure Localization*), classificação da categoria da falha (*Failure Category Classification*) e geração de relatório de causa raiz (*Root Cause Report Generation*). Esta última subtarefa emergiu com a integração de LLMs e representa a contribuição central deste trabalho — o sistema proposto recebe o contexto de um incidente e gera, em linguagem natural, um diagnóstico explicável da causa provável da falha.

**Assisted Remediation** (Remediação Assistida) é o estágio final, responsável por mitigar e resolver o problema identificado. Zhang et al. (2025) classificam as abordagens de remediação em cinco níveis crescentes de automação: questionamento assistido, geração de solução de mitigação, recomendação de comando, geração de script e execução automática. Wang et al. (2025) demonstram que sistemas com remediação automatizada são capazes de resolver 91% dos incidentes sem intervenção humana, porém ressaltam que mecanismos de validação de segurança e supervisão humana (*human-in-the-loop*) permanecem essenciais para sistemas críticos.

No presente trabalho, o escopo é deliberadamente delimitado ao estágio de RCA, mais especificamente à subtarefa de geração de relatório de causa raiz. O sistema não executa ações de remediação automática — o diagnóstico gerado pelo LLM é apresentado ao operador humano, que decide a ação a tomar. Essa delimitação é justificada por três razões: (1) a remediação automática em infraestrutura de produção exige validação extensiva e mecanismos de *rollback* que estão fora do escopo de uma prova de conceito; (2) o foco no diagnóstico em linguagem natural representa, por si só, uma contribuição original na integração com o *stack* Prometheus e LGTM; e (3) Wang et al. (2025) apontam a remediação automática como a área com menor maturidade na literatura de AIOps com LLMs.

---

## 2.2.3 Evolução: do AIOps tradicional ao AIOps baseado em LLMs

As abordagens tradicionais de AIOps, baseadas em aprendizado de máquina clássico e aprendizado profundo, desempenharam papel relevante na automação de operações de TI ao longo da última década. Notaro et al. (2021) apresentam um panorama abrangente dessas abordagens, cobrindo técnicas de percepção de falhas, análise de causa raiz e remediação com algoritmos supervisionados, não supervisionados e de aprendizado por reforço.

Entretanto, Zhang et al. (2025) identificam cinco limitações estruturais que restringem a eficácia dessas abordagens em ambientes modernos:

**Necessidade de engenharia de *features* complexa.** Métodos tradicionais exigem extenso pré-processamento e extração de características dos dados brutos. Sua capacidade de compreensão de dados não estruturados — como logs em texto livre — é limitada, demandando pipelines de transformação específicos para cada tipo de dado e sistema.

**Falta de generalidade entre plataformas.** Modelos treinados para um sistema específico degradam significativamente quando aplicados a sistemas diferentes ou quando o sistema original passa por mudanças, mesmo que pequenas. Isso implica necessidade frequente de retreinamento, tornando as soluções pouco escaláveis em ambientes heterogêneos.

**Falta de flexibilidade entre tarefas.** Devido à especificidade do conhecimento e das saídas de cada modelo, abordagens tradicionais conseguem executar apenas uma tarefa por vez. Em cenários reais de RCA, múltiplos modelos precisam ser executados simultaneamente para cobrir diferentes aspectos do diagnóstico.

**Adaptabilidade limitada.** Mudanças no sistema exigem retreinamento frequente para que os modelos se adaptem a novos dados e ambientes. Embora existam métodos de aprendizado online, esse processo consome tempo e pode resultar em respostas atrasadas diante de eventos repentinos.

**Níveis restritos de automação.** Métodos baseados em aprendizado profundo apresentam capacidades relativamente limitadas de operação autônoma e tomada de decisão inteligente, ainda exigindo intervenção e configuração manual significativas — especialmente nas tarefas de remediação.

É em resposta a essas limitações que os LLMs emergem como abordagem promissora para AIOps, conforme explorado na seção 2.3. A Tabela 2.X sintetiza a evolução das abordagens de AIOps e posiciona o presente trabalho nesse contexto.

**Tabela 2.X — Evolução das abordagens de AIOps**

| Característica | AIOps tradicional (ML/DL) | AIOps com LLMs | Este trabalho |
|---|---|---|---|
| Dados não estruturados | ⚠️ Requer pré-processamento | ✅ Nativo | ✅ Logs via Loki |
| Generalidade entre plataformas | ❌ Limitada | ✅ Alta | ✅ Stack LGTM + AWS |
| Múltiplas tarefas simultâneas | ❌ Um modelo por tarefa | ✅ Flexível | ✅ RCA multimodal |
| Adaptação sem retreinamento | ❌ Exige retreinamento | ✅ Via RAG/Few-shot | ✅ Injeção de contexto |
| Explicabilidade em linguagem natural | ❌ Ausente | ✅ Nativa | ✅ Diagnóstico em PT-BR |
| Execução local sem GPU dedicada | ✅ Viável | ⚠️ Depende do modelo | ✅ Ollama + API fallback |

---

## 2.2.4 AIOps no contexto de infraestrutura AWS e stack open source

A adoção de AIOps em ambientes corporativos enfrenta uma tensão recorrente entre capacidade técnica e viabilidade econômica. Soluções comerciais como Datadog AI Ops, Dynatrace Davis e AWS DevOps Guru oferecem funcionalidades avançadas de correlação e diagnóstico, porém com custos de licenciamento que as tornam inacessíveis para empresas de médio porte [Zhang et al., 2025]. Adicionalmente, essas plataformas exigem o envio de dados operacionais para infraestruturas externas, o que pode representar impedimentos de conformidade e segurança da informação.

No outro extremo, iniciativas *open source* como o Keep (keephq) avançam na direção da correlação automatizada de alertas, porém com foco em sumarização e integração com APIs de LLMs externos — sem suporte nativo a execução local de modelos e sem integração profunda com dados de logs para RCA [Zhang et al., 2025].

O presente trabalho posiciona-se nessa lacuna: propõe uma arquitetura *open source*, construída sobre o *stack* Prometheus e LGTM já amplamente adotado em ambientes AWS, que integra LLMs para diagnóstico de causa raiz em linguagem natural. A flexibilidade de implantação — suportando tanto execução local via Ollama quanto consumo de APIs de inferência como Groq e Gemini — torna a solução acessível a diferentes perfis de infraestrutura e orçamento, sem dependência de plataformas proprietárias.

---

## 2.2.5 Métricas de avaliação em sistemas AIOps

A avaliação de sistemas AIOps envolve dimensões técnicas e operacionais distintas. Zhang et al. (2025) identificam quatro categorias de métricas utilizadas na literatura com LLMs: métricas de tarefa de classificação (precisão, recall, F1-score), métricas de tarefa de geração (BLEU, ROUGE, BERTScore), métricas de tarefa de execução (taxa de sucesso de remediação) e avaliação manual (avaliação qualitativa por especialistas humanos).

Wang et al. (2025) adotam uma abordagem pragmática, avaliando seu sistema em quatro dimensões: desempenho de detecção (precisão, recall, F1-score), tempo médio de resolução (MTTR), taxa de automação e *overhead* de sistema. Os resultados obtidos — redução de 84,2% no MTTR em relação a métodos baseados em regras e F1-score de 0,95 na detecção de anomalias — evidenciam o potencial da abordagem para ambientes de produção.

No presente trabalho, a avaliação do protótipo adota métricas compatíveis com o escopo de RCA: acurácia qualitativa do diagnóstico (avaliação humana), latência de resposta do sistema e relevância do contexto utilizado pelo LLM. Essa escolha é justificada pela ausência de um *ground truth* estruturado — os diagnósticos gerados em linguagem natural não possuem uma resposta única e verificável automaticamente, sendo mais adequada a avaliação qualitativa por especialistas, conforme recomendado por Zhang et al. (2025) para tarefas de geração de relatório de causa raiz.
...
| Chave | Referência |
|---|---|
| [Prasad e Rich, 2018] | PRASAD, Pankaj; RICH, Charley. **Market guide for AIOps platforms**. Gartner, 2018. p. 2–9. |
